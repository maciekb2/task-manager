...
func TestRun_Success(t *testing.T) {
	...
	// dispatchLoop and processLoop run in goroutines and will exit on context cancel
	// Ideally we wait for them, but run() blocks on ctx.Done() then waits 1 second.
	// We might want to shorten that wait time for tests, but it's hardcoded.
	// Test will take ~1s.
...
func TestRun_ConsumerFail(t *testing.T) {
	mockBus := new(MockBusClient)
	mockBus.On("EnsureStream", mock.Anything).Return(&nats.StreamInfo{}, nil)
	mockBus.On("EnsureConsumer", mock.Anything, mock.Anything).Return(nil, errors.New("consumer fail")).Once()
	mockBus.On("Close").Return()
...
func TestRun_SubscribeFail(t *testing.T) {
	...
	mockBus.On("EnsureStream", mock.Anything).Return(&nats.StreamInfo{}, nil)
	mockBus.On("EnsureConsumer", mock.Anything, mock.Anything).Return(&nats.ConsumerInfo{}, nil)
	mockBus.On("PullSubscribe", mock.Anything, mock.Anything, mock.Anything).Return(nil, errors.New("sub fail")).Once()
	mockBus.On("Close").Return()
