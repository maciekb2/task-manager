...
func TestSubmitTask_UnreachableTarget(t *testing.T) {
	addr := os.Getenv("TASKMANAGER_ADDR")
	if addr == "" {
		addr = "envoy:50051"
	}
	...
	// Submit Task with Unreachable URL
	var res *pb.TaskResponse
	for i := 0; i < 10; i++ {
		subCtx, subCancel := context.WithTimeout(context.Background(), 5*time.Second)
		res, err = client.SubmitTask(subCtx, &pb.TaskRequest{
	...
	// We expect the task to complete (gracefully handle failure)
	// The status will likely be COMPLETED (but result code inside system is error)
	// or FAILED (if the system counts network error as fatal).
	// Based on worker.go analysis, it returns COMPLETED.
	for {
		status, err := stream.Recv()
		if err != nil {
			t.Fatalf("Error receiving status: %v", err)
		...
		log.Printf("Task Status: %s", status.Status)
		if status.Status == "COMPLETED" || status.Status == "FAILED" {
			log.Printf("Task finished (as expected) with status: %s", status.Status)
			break
		}
	}
}
